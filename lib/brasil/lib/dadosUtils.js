...
function obterBancosComCodigo() {
    return bancosArray.filter(function(banco) {
        return banco.codigo;
    });
}
...
function obterFeriados(uf, dataInicial, dataFinal) {
    var feriadosEncontrados = [],
        feriadosNacionais = feriadosNacionaisArray;

    feriadosEncontrados = feriadosEncontrados.concat(feriadosNacionais);

    if(typeof uf !== 'undefined') {
        var feriadosEstaduais = feriadosEstaduaisHash;
        uf = uf.toLowerCase();
...
function eFeriado(data, uf) {
    var feriadosNacionais = feriadosNacionaisArray,
    	eFeriado = false;
